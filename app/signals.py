...
class SignalService:
    ...
    async def _process(self, raw):
        ...
        setup = raw.get("setup") or raw["detector"]
        ...
        log.info("SIGNAL     %-14s %-18s ar %.8g  %s",
                 symbol, setup, raw["price"], binance_url(symbol))
        events.add(f"{symbol:<14} SIGNAL {setup}")
