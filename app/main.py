...
def startup_summary(cfg):
    ...
    return ([f"A DB-ben eltero beallitas: {', '.join(eltero)}"] if eltero else []) + [
        ...
        f"1. IMPULZUS: a mozgas a par normaljanak {d['impulseBaselineRatio']:.1f}x-e "
        f"(min {d['minImpulsePct']:.2f}%), forgalom >= {d['minImpulseNotional']:,.0f} "
        f"USDT es a normal {d['notionalRatio']:.1f}x-e, kotesaramlas >= "
        f"{d['minImpulseImbalance']:.2f}  ({d['impulseWindowSec']:.0f} mp-es ablak)",
        f"2. PULLBACK: {d['minPullbackPct']}-{d['maxPullbackPct']}% a labbol, "
        f"a setup {d['setupTimeoutSec']}s-ig el",
        f"3. FRISS KITORES: {d['breakoutOfLegPct']}%-os attores, max "
        f"{d['maxBreakoutAgeSec']:.0f} mp regi, es max {d['maxEntryExtensionPct']}%-ra "
        f"a szinttol",
        f"4. MEGEROSITES: kotesaramlas >= {d['minConfirmImbalance']:.2f} es FRISS "
        f"konyv-adat (max {d['maxDataAgeSec']:.0f} mp) -- enelkul NINCS jelzes",
